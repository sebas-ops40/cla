...
static void FlyCommand_Execute(const cc_string* args, int argsCount) {
	...
	if (argsCount == 0) {
		/* Sin argumentos: alternar vuelo */
		HacksComp_SetFlying(hacks, !hacks->Flying);
		if (hacks->Flying)
			Chat_AddRaw("&aVuelo activado. &eUsa /fly off para desactivar.");
		else
			Chat_AddRaw("&cVuelo desactivado.");
		return;
	}

	if (String_CaselessEqualsConst(&args[0], "on")) {
		HacksComp_SetFlying(hacks, true);
		Chat_AddRaw("&aVuelo activado. &eUsa /fly off para desactivar.");
	} else if (String_CaselessEqualsConst(&args[0], "off")) {
		HacksComp_SetFlying(hacks, false);
		Chat_AddRaw("&cVuelo desactivado.");
	} else {
		Chat_AddRaw("&e/client fly: &cUso invalido. Escribe /fly, /fly on o /fly off.");
	}
}
...
static struct ChatCommand FlyCommand = {
	...
	{
		"&a/client fly",
		"&eActiva o desactiva el modo vuelo.",
		"&a/client fly [on/off]",
		"&eActiva o desactiva el vuelo explicitamente.",
	}
};
...
static void GiveCommand_Execute(const cc_string* args, int argsCount) {
	...
	if (argsCount == 0) {
		Chat_AddRaw("&e/client give: &cDebes especificar un bloque.");
		Chat_AddRaw("&eUso: &a/client give [nombre o id de bloque]");
		return;
	}
	...
	if (block == -1) {
		Chat_Add1("&e/client give: &c\"%s\" no es un nombre o ID de bloque valido.", &args[0]);
		return;
	}

	if (block > Game_Version.MaxCoreBlock && !Block_IsCustomDefined(block)) {
		Chat_Add1("&e/client give: &cNo existe un bloque con id \"%s\".", &args[0]);
		return;
	}

	Inventory_SetSelectedBlock((BlockID)block);
	Chat_Add1("&eTienes ahora: &a%s &een tu mano.", &args[0]);
}
...
static struct ChatCommand GiveCommand = {
	...
	COMMAND_FLAG_SINGLEPLAYER_ONLY | COMMAND_FLAG_UNSPLIT_ARGS,
	{
		"&a/client give [bloque]",
		"&ePone el bloque indicado en tu mano (slot seleccionado).",
		"&ePuedes usar el nombre del bloque o su ID numerico.",
	}
};
